...
namespace CppLmpc{
    ...
    CppLmpc::CppLmpc(double sampling_time, const MatrixXd& Ac, const MatrixXd& Bc, const MatrixXd& Cc, const MatrixXd& Dc)
    ...
    Dc_(Dc){
        ...
        int Hp = 75;  // prediction horizon
        int Hu = Hp;  // control horizon
        int Hw = 0;   // window parameter

        MatrixXd Ad;
        MatrixXd Bd;
        MatrixXd Cd = Cc_;
        MatrixXd Dd = Dc_;
        CppControl::c2d(Ac_, Bc_, sampling_time_, Ad, Bd);

        Ap_ = calcAp(Ad, Hp, Hw);
        Bp1_ = calcBp1(Ad, Bd, Hp, Hw, Hu);
        Bp2_ = calcBp2(Ad, Bd, Hp, Hw, Hu);
        Cp_ = calcCp(Ad, Bd, Cd, Hp, Hw);
        ...
        /*
        W_ = calcInputRateIneqMat_1(du_max, du_min, Hu);
        w_ = calcInputRateIneqMat_2(du_max, du_min, Hu);

        Aineq_ = calcInputIneqMat_1(u_max, u_min, Hu);
        Aineq1_ = calcInputIneqMat_2(u_max, u_min, Hu);
        Aineq_bias_ = calcInputIneqMat_3(u_max, u_min, Hu);

        Gamma_ = calcOutputIneqMat_1(z_max, z_min, Hp);
        gamma_ = calcOutputIneqMat_2(z_max, z_min, Hp);
        H_ = calcStateIneqMat_1(x_max, x_min, Bd.rows(), Hp);
        eta_ = calcStateIneqMat_2(x_max, x_min, Bd.rows(), Hp);

        Tau_ = MatrixXd::Zero(Hp-Hw, 1); // Tr
        Bstate_ = Bp2;
        Hess_ = Theta.transpose() * Q * Theta + Bstate.transpose() * S * Bstate + R;
        Gineq_ = Gamma * Theta;
        Hineq_ = H * Bp2;
        */
    };
